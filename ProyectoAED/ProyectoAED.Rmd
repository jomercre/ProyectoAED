...
# para documentar como ha ido la imputación podemos generar una tabla con los porcentajes de imputación y importante quitar de nuevo los casos na no imputables
imputaciones_por_rama <- df %>%
  filter(TRBPRN1 == "Trabajando") %>%
  group_by(RAMA) %>%
  summarise(
    Total_Registros = n(),
    Imputados = sum(is.na(TR_SUELDO)),
