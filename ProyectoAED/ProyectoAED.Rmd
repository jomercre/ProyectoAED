...
print("Valores faltantes en TR_SUELDO 
      después de la imputación (solo trabajando)")
...
# unir tablas
resumen_imputacion <- left_join(moda_por_rama,
                                imputaciones_por_rama, by = "RAMA")
...
```{r, echo=FALSE}
tabla_NA_rama <- as.data.frame.matrix(table(df$RAMA, is.na(df$TR_SUELDO)))
...
ggplot(df, aes(x = RAMA, fill = SEXO)) +
  ...
  labs(
    title = 
    "Distribución de mujeres y hombres por rama de estudio",
    x = "Rama de estudio",
    y = "Proporción",
    fill = "Sexo"
  ) +
  scale_fill_manual(values = c("#4169E1", "#E4007C")) +
  theme_minimal(base_size = 12) +
  theme(axis.text.x = element_text(angle = 20, hjust = 1))
...
```{r figT9,echo=FALSE, fig.cap="Distribución salarial por tipo de universidad",fig.align = 'center', out.width = "0.6\\linewidth"}

ggplot(df %>% filter(TRBPRN1 == "Trabajando", !is.na(T_UNIV),
                     !is.na(TR_SUELDO)),
...
```
El objetivo de esta gráfica es analizar la relación entre el tipo de universidad cursada *T_UNIV* y el nivel salarial actual *TR_SUELDO* de las personas que se encuentran trabajando. El gráfico permite comparar la estructura salarial de los egresados según el tipo de universidad. En general, se observa que las universidades públicas a distancia tienden a concentrar mayores proporciones de individuos en los tramos salariales altos. Por el contrario, las universidades públicas presenciales  suelen mostrar una mayor proporción de sueldos en los tramos bajos o medios. La universidad privada, tanto a distancia como presencial, se encuentran en un perfil intermedio más equilibrado.
...
df$IDIOMAS <- factor(df$IDIOMAS,
                     ...
                     labels = c("Un estudio", "Dos estudios", "Tres",
                                "Cuatro", "Cinco o más"),
                     ordered = TRUE)
