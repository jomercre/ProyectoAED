...
El proceso que se seguirá es una imputación de los datos presentes en la variable:

En primer lugar, se verifica que la variable RAMA se encuentre definida como un factor nominal y que T_SUELDO se establezca como un factor ordinal. A continuación, se realiza una exploración inicial para cuantificar el número total de valores faltantes (NA) y determinar su distribución por grupo de rama.

Posteriormente, se procede con la imputación condicional, sustituyendo los valores faltantes de T_SUELDO por la mediana ordinal correspondiente al grupo definido por la variable RAMA, excluyendo los valores ausentes. Una vez completada esta etapa, la variable imputada se reconvierte en un factor con las etiquetas originales.

Finalmente, se genera un gráfico de barras apiladas que representa las proporciones de los distintos rangos salariales por rama de estudio, utilizando los datos tras la imputación.


```{r}

#aquí imputamos usando dplyr y count porque la moda no la podemos sacar de otra forma

library(dplyr)
library(ggplot2)

# primero convertimos las variables que vamos a usar a factore
...
# verificar las variables que faltan los na


cat("Número de valores faltantes en TR_SUELDO (antes):\n")
sum(is.na(df$TR_SUELDO))

cat("\nDistribución de NAs por rama (antes):\n")
print(table(df$RAMA, is.na(df$TR_SUELDO)))


# MODA POR RAMA SOLO PERSONAS TRABAJANDO y quitando los NS/NC
...
# imputar condicionalmente pero solo personas que sí trbajan ya que si no trabajan no aportan información valida real
...
#  convertir TR_SUELDO a factor ordenado con sus niveles de antes.
df$TR_SUELDO <- factor(df$TR_SUELDO,
                       levels = c("Menos de 700 euros",
                                  "De 700 a 999 euros",
                                  "De 1.000 a 1.499 euros",
...
# verificación results

cat("\nValores faltantes en TR_SUELDO después de la imputación (solo trabajando):\n")
sum(is.na(df$TR_SUELDO[df$TRBPRN1 == "Trabajando"]))

cat("\nFrecuencia de niveles TR_SUELDO (en todo el dataset):\n")
print(summary(df$TR_SUELDO))

# visualizar como ha quedaod
...
# para documentar como ha ido la imputación podemos generar una tabla.
imputaciones_por_rama <- df %>%
  filter(TRBPRN1 == "Trabajando") %>%
  group_by(RAMA) %>%
  summarise(
    Total_Registros = n(),
    Imputados = sum(is.na(TR_SUELDO)),  # Los que aún son NA (no imputables)
    Completos = Total_Registros - Imputados
  ) %>%
  mutate(
    Porcentaje_Imputado = round((Total_Registros - Imputados) / Total_Registros * 100, 2)
  )

#unir tablas (moda y recuento)
resumen_imputacion <- left_join(moda_por_rama, imputaciones_por_rama, by = "RAMA")
resumen_imputacion

```
La imputación condinional aplicada ha funcionado correctamente al imputar un total de 4527 datos faltantes usando la mediana.
La categoría “1.000–1.499 euros” sigue siendo la más frecuente esto suele ser realista y consistente con distribuciones salariales típicas.
El número de casos imputados (4 527) se ha redistribuido de forma coherente con la estructura de los datos.
No hay picos o saltos anómalos en ninguna categoría, lo cual indica que la imputación no sesgó visiblemente la distribución.

Obtenemos información sobre la mediana imputada por cada rama de estudio, lo cual corresponde con el nivel salarial típico que ha usado el algoritmo para imputar los datos faltantes.

```{r}

#MEDIANA IMPUTADA POR RAMA

library(dplyr)

medianas_por_rama <- df %>%
  group_by(RAMA) %>%
  summarise(
    Mediana_Sueldo = levels(TR_SUELDO)[median(as.numeric(TR_SUELDO))],
    n = n()
  )

print(medianas_por_rama)

```


La siguiente tabla muestra todas las ramas con valores salariales distribuidos tras la imputación.
