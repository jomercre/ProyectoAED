...
# IMPUTACIÓN DE T_SUELDO SEGÚN RAMA DE ESTUDIO

library(dplyr)
library(ggplot2)


# convertimos las variables a factores (TR_SUELDO y RAMA)
...
cat("Número de valores faltantes en T_SUELDO:\n")
sum(is.na(df$TR_SUELDO))

cat("\nDistribución de NAs por rama:\n")
table(df$RAMA, is.na(df$TR_SUELDO))


# Imputacion por rama.
# Convertimos TR_SUELDO a índice numérico ordenado para calcular la mediana de cada grupo
df <- df %>%
  group_by(RAMA) %>%
  mutate(
    TR_SUELDO = ifelse(
      is.na(TR_SUELDO),
      as.numeric(stats::median(as.numeric(TR_SUELDO), na.rm = TRUE)),
      as.numeric(TR_SUELDO)
    )
  ) %>%
  ungroup()

# reconstruimos factor del suelkdo ordenado con etiquetas originales.
df$TR_SUELDO <- factor(round(df$TR_SUELDO),
                      levels = 1:8,
                      labels = c("Menos de 700 euros",
                                 "De 700 a 999 euros",
                                 "De 1.000 a 1.499 euros",
                                 "De 1.500 a 1.999 euros",
                                 "De 2.000 a 2.499 euros",
...
cat("\nFrecuencia de niveles T_SUELDO:\n")
print(summary(df$TR_SUELDO))


# visualizamos las distribucione despues de la imputación.

ggplot(df, aes(x = RAMA, fill = TR_SUELDO)) +
  geom_bar(position = "fill") +
  labs(
    title = "Distribución de niveles salariales imputados por rama de estudio",
    x = "Rama de estudio",
