...
Como podemos observar las únicas columnas con datos faltantes son IDIOMAS, TR_SUELDO y HATR_E11. La variable con más datos faltantes es TR_SUELDO con un ```r sum(is.na(df$TR_SUELDO))/nrow(df)*100``` % de datos con NA. No obstante, no todos ellos son datos que no han sido rellenados, ya que puede darse el caso de que la persona no esté actualmente trabajando y, por tanto, no haya rellenado a drede la casilla de TR_Sueldo. Para observar cuales son en realidad nuestros datos faltantes procedemos a observar el número de NA's de la variable TR_SUELDO una vez hemos eliminado aquellas personas que no se encuentran trabajando.
```{r tab2, echo=FALSE}
...
```{r figT1, fig.cap="Gráfico de barras de RAMA/URAMA coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
...
```{r figT2, fig.cap="Gráfico de barras de TRBPRN1 coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
...
```{r figT3, fig.cap="Gráfico de barras de TRBPRN1 coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
...
```{r figT4, fig.cap="Gráfico de barras de TRBPRN1 coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
ggplot(data=df, aes(x = HATR_E11)) + geom_bar(aes(fill=T_UNIV)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10)) +
  scale_fill_brewer(palette = "YlOrRd")
```
...
```{r figT5, fig.cap="Gráfico de barras de TRBPRN1 coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
ggplot(data=df, aes(x = HATR_E11)) + geom_bar(aes(fill=T_UNIV)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10)) +
  scale_fill_brewer(palette = "YlOrRd")
```
```{r figT6, fig.cap="Gráfico de barras de TRBPRN1 coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
...
```{r figT7, fig.cap="Gráfico de barras de TRBPRN1 coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
ggplot(df, aes(x= TRBPRN1)) + geom_bar(aes(fill=URAMA)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10)) +
  scale_fill_brewer(palette = "Purples")
```

Número de idiomas frente a situación laboral.
```{r figT8, fig.cap="Gráfico de barras de TRBPRN1 coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
...
```{r figT9, fig.cap="Gráfico de barras de TRBPRN1 coloreado con SEXO",fig.align = 'center', out.width = "0.8\\linewidth"}
ggplot(df, aes(x=URAMA)) + geom_bar(aes(fill=SEXO)) + facet_wrap(~TRBPRN1) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
```
